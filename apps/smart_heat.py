...
class SmartHeat(hass.Hass):
  ...
  def initialize(self):
    ...
    # Subscribe to temperature inputs
    self.listen_state(self.on_temp_change, self.args["on_temp"])
    self.listen_state(self.on_temp_change, self.args["off_temp"])
  ...
  def switch(self, entity, attribute, old, new, kwargs):
    # Toggling switch turns heat on and off as well as enabling smart behaviour
    if new == "on":
      self.heat_on()
    else:
      self.heat_off()
  ...
  def set_temperature(self, temperature):
    self.log("Changing temperature to: {}°C".format(temperature))
    self.notify("Changing temperature to: {}°C".format(temperature), name=config.notify)    
    for tstat in self.split_device_list(self.args["thermostats"]):
        self.call_service("climate/set_temperature", entity_id = tstat, temperature = temperature)
      
  def heat_on(self):
    if self.state == "off":
      self.state = "on"
      self.log("Turning heat on")
      self.notify("Turning heat on", name=config.notify)
      self.set_temperature(self.get_state(self.args["on_temp"]))
      
  def heat_off(self):
    if self.state == "on":
      self.state = "off"    
      self.log("Turning heat off")
      self.notify("Turning heat off", name=config.notify)
      self.set_temperature(self.get_state(self.args["off_temp"]))
        
  def on_temp_change(self, entity, attribute, old, new, kwargs):
    temp = "0"
    if entity == self.args["off_temp"] and self.state == "off":
      temp = self.get_state(self.args["off_temp"])
    if entity == self.args["on_temp"] and self.state == "on":
      temp = self.get_state(self.args["on_temp"])
